...
def run_experiment_2(n_trials: int, db: Database, config: Config) -> Dict[str, Any]:
    """
    Experiment 2: Progressive Layer Activation (RQ1)
    
    Tests layer-by-layer security improvement with 6 configurations:
    - Config A: No Defense (baseline)
    - Config B: Layer 1 only
    - Config C: Layers 1-2
    - Config D: Layers 1-2-3
    - Config E: Layers 1-2-3-4
    - Config F: Full Stack (all 5 layers)
    
    Per STATISTICAL_REQUIREMENTS.md
    """
    logger.info("="*80)
    logger.info("EXPERIMENT 2: Progressive Layer Activation (RQ1)")
    logger.info(f"Running with {n_trials} trial(s) per configuration")
    logger.info(f"Testing all {len(ATTACK_PROMPTS)} attack prompts + {len(BENIGN_PROMPTS)} benign prompts")
    logger.info("="*80)
    
    experiment_id = "exp2_progressive_layers"
